...
class TestSelectFeatures:
    ...
    def test_persist_importance(self, fe, sample_features_df, session_factory):
        ...
        selected = fe.select_features(
            sample_features_df, max_features=10,
            session_factory=session_factory, stock_id="2330",
        )
        ...
        try:
            records = session.query(FeatureImportanceRecord).filter_by(
                stock_id="2330",
            ).all()
            ...
            records2 = session.query(FeatureImportanceRecord).filter_by(
                stock_id="2330",
            ).all()
            # 第二次跑完後的紀錄數應和第一次相同
...
class TestPrepareSequences:
    def test_output_shape(self, fe, sample_features_df):
        ...
        X, y = fe.prepare_sequences(
            sample_features_df, seq_len=10, feature_cols=feature_cols,
    ...
    def test_dtype_float32(self, fe, sample_features_df):
        X, y = fe.prepare_sequences(
            sample_features_df, seq_len=5,
            feature_cols=["close"], target_col=TB_TARGET_COLUMN,
        )
...
class TestPrepareTabular:
    def test_output_shape(self, fe, sample_features_df):
        ...
        X, y = fe.prepare_tabular(
            sample_features_df, feature_cols, target_col=TB_TARGET_COLUMN,
        )
        assert X.ndim == 2
