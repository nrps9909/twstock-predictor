...
class RiskManager:
    ...
    def __init__(
        ...
        # Persistent state file for circuit breaker
        self._state_file = Path(__file__).resolve().parent.parent.parent / "data" / "risk_state.json"
        self._load_persistent_state()

    def _load_persistent_state(self):
        """Load circuit breaker state from file (survives restarts)."""
        try:
            if self._state_file.exists():
                state = json.loads(self._state_file.read_text())
                self._circuit_breaker_active = state.get("circuit_breaker_active", False)
                if self._circuit_breaker_active:
                    logger.warning("Circuit breaker state restored: ACTIVE")
        except Exception as e:
            logger.debug("Could not load risk state: %s", e)
    ...
    def _save_persistent_state(self):
        ...
        try:
            ...
            self._state_file.write_text(
                json.dumps({"circuit_breaker_active": self._circuit_breaker_active})
            )
        except Exception as e:
            logger.debug("Could not save risk state: %s", e)
    ...
    def init_trailing_stop(
        ...
        Args:
            ...
            entry_price: 進場價格
            atr: 當前 ATR
            multiplier: ATR 倍數（預設 3.0x — 台股日均波動 1-2.5%，需較寬停損避免噪音）
