...
def generate_alerts_from_scan(
    ...
    for stock in current_results:
        ...
        if prev_signal and signal != prev_signal:
            ...
            is_flip = (
                (prev_signal in sell_signals | {"hold"} and signal in buy_signals) or
                (prev_signal in buy_signals | {"hold"} and signal in sell_signals)
            )
            if is_flip:
    ...
    if high_alert_stocks:
        try:
            ...
            if loop.is_running():
                ...
                logger.info("Triggered reanalysis for %d high-severity stocks", len(high_alert_stocks))
        except Exception as e:
